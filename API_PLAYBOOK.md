...
- **Version:** `GET /api/info` → `{"version": "0.24.0"}`, zur Laufzeit aus `server/package.json` gelesen
...
**Dauerhaft ausschließen (`excludedRows`):** `commit` nimmt optional `excludedRows: [{date, amountCents, description}]` entgegen (`rows` darf dann leer sein) und speichert sie in derselben Transaktion als Ausschlüsse für `defaultAccountId`. `preview` markiert passende Zeilen später mit `excluded: true` und `exclusionId` (Vorschläge/Doubletten-Prüfung entfallen, `commit` darf sie nicht enthalten). Abgleich über Fingerabdruck aus Ziel-Konto + Datum + Betrag + normalisierter Beschreibung (klein, Whitespace zusammengefasst) — **pro Ziel-Konto**, keine Bank-Transaktions-ID im generischen CSV-Format. Identische Doppelzeilen werden als mehrere Einträge gespeichert und pro Eintrag nur einmal ausgeschlossen. Anders als `skipPatterns` (Regex-Regel, gehört zur Vorlage) ist das eine Entscheidung über *einzelne* Zeilen. `GET /import/exclusions` listet alle (`{id, accountId, accountName, date, amountCents, description}`), `DELETE /import/exclusions/{id}` hebt einen Ausschluss auf.
