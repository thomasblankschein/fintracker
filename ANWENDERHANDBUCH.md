...
③ **Buchungsliste** — jede Zeile zeigt Datum, Zahlungsempfänger, Beschreibung und alle beteiligten Konten mit Betrag. Unterhalb der Liste lässt sich die **Seitengröße** wählen (25/50/100/200 Buchungen pro Seite) und mit „Zurück"/„Weiter" blättern — hält die Seite auch bei vielen hundert Buchungen schnell. Der CSV-Export (siehe unten) ist davon unabhängig und umfasst immer die komplette gefilterte Menge, nicht nur die aktuell sichtbare Seite.
...
**Zeilen dauerhaft ausschließen:** Zeilen, die du bewusst nicht buchen willst, kannst du in Schritt 3 per **„Dauerhaft ausschließen"** markieren (Button unter dem Kategorie-Feld). Beim Import werden sie nicht gebucht, aber gemerkt — importierst du dieselbe Datei später erneut in dasselbe Konto, erscheinen sie automatisch als **„Ausgeschlossen"** (gedimmt, ohne Kategorie-Auswahl) statt wieder zur Zuordnung. Mit **„Freigeben"** hebst du den Ausschluss wieder auf. Wiedererkannt wird eine Zeile an Datum, Betrag und Beschreibung (Groß-/Kleinschreibung und Leerzeichen egal, maßgeblich ist der ursprüngliche CSV-Text, nicht ein von dir im Vorschau-Feld ergänzter) — und nur innerhalb desselben Ziel-Kontos. Das ist etwas anderes als „Keine Zuordnung", das nur „jetzt nicht entschieden" bedeutet und beim nächsten Import wieder zur Auswahl steht.
